...
for param in job_params:
    jobs.append({
        ...
        "gkmexplain_base_path": param["gkmexplain_base_path"],
...
for job in jobs:
    ...
    cmd += ["./generate_importance_scores.sh"]
    cmd += [str(x) for x in [job["cell_type"], job["fold_id"], job["model_base_path"], \
        job["gkmexplain_base_path"], job["snp_sequence_base_path"], job["gkmsvm_base_path"], \
        job["broad_cluster"]]]
