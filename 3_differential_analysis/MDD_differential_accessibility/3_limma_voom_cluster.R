...
obj <- readRDS("/home/anjali5/projects/def-gturecki/anjali5/ArchR_cluster_by_peaks.rds") #cluster
...
sce <- prepSCE(sce,kid = "SubClusters",gid = "condition",sid = "Subject",drop=FALSE)
nk <- length(kids <- levels(sce$cluster_id))
ns <- length(sids <- levels(sce$sample_id))
names(kids) <- kids; names(sids) <- sids
t(table(sce$cluster_id, sce$sample_id))
#saveRDS(sce,"~/scratch/sce_fine.rds")
...
saveRDS(pb, "~/projects/def-gturecki/anjali5/Finalized_DAR/subcluster/pb_fine.rds")
#pb <- readRDS("~/projects/def-gturecki/anjali5/Finalized_DAR/subcluster/pb_fine.rds")
#pb
#check_samples
...
result <- pbDS(pb, design = design, contrast = cm, method = "limma-voom", min_cells=5, filter = "both") #min_cells=10 for broad cell-type 

#save results
saveRDS(result, "~/scratch/subcluster_result_limma_allPeaks_corrected_filtered.rds")

##Recheck with Matt's method (gives equivalent result)
...
#filter results to keep peaks accessible in an average of 3% of cells in at least 1 group
#Load result file
...
write.csv(atac_sub, "~/scratch/DARs_cluster.csv")
