...
##Get microglia clusters##############
Idents(ArchR) <- "SubClusters"
ArchR <- subset(ArchR, idents=c("Mic1","Mic2"))
ArchR <- SCTransform(ArchR, assay = "RNA") %>% RunPCA(verbose = FALSE, npcs = 50)
...
#find transfer anchors between ATAC and spatial and make predictions 
anchors <- FindTransferAnchors(reference = ArchR, query = spatial, normalization.method = "SCT",npcs = 30)
#save.image(file = "/home/anjali5/scratch/graham_merge_analysis/merged_Rdata/ArchR_spatial.RData")
...
saveRDS(spatial,paste0("/home/anjali5/scratch/SpatialVisualization/mergedSections_PromoterAcc_Mic_Spatial.rds"))
print("DONE")
