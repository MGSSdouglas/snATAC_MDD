@adapted from ArchR (Granja et. al, 2021) https://www.nature.com/articles/s41588-021-00790-6
# https://github.com/AnjaliC4/ArchR_plots/blob/main/Highlight_ArchRTracks.R
#PS: https://github.com/GreenleafLab/ArchR/discussions/1342
  ...
  if(nrow(genesO) > 0){
    ...
    if(length(which(genesO$strand!="-")) > 0){
      p <- p + ggrepel::geom_label_repel(data=genesO[which(genesO$strand!="-"),], 
                                         ...
                                         size = labelSize, direction = "x",inherit.aes=F)
    }
