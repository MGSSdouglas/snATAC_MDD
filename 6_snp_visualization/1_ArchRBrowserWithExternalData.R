...
#' 
#' @export
plotBrowserTrackWithExternalData <- function(
  ArchRProj = NULL, 
  region = NULL, 
  groupBy = "Clusters",
  useGroups = NULL, 
  #update next two lines
  plotSummary = c("bulkTrack", "featureTrack", "loopTrack", "geneTrack"), #edited. by default
  sizes = c(10, 1.5, 3, 4), #must correspond to elements in plotSummary above
  features = getPeakSet(ArchRProj),
  #new
  external_features = NULL, #add in an extra argument to represent external peak sets (in the form of a VECTOR of GRanges objects)
  external_feature_names = NULL, #pass in the names of the external peak sets (in same order as external_features)
  loops = getCoAccessibility(ArchRProj),
  #new
  external_loops = NULL, #pass in a list of GRanges objects 
  external_loop_names = NULL, #pass in the names of the external loop tracks (in same order as external_loops)
  geneSymbol = NULL,
  useMatrix = NULL,
  log2Norm = TRUE,
  upstream = 50000,
  ...
  #add some validInput checks for our external features - this hasn't been done yet

  tstart <- Sys.time()
  .startLogging(logFile=logFile)
  .logThis(mget(names(formals()),sys.frame(sys.nframe())), "plotBrowserTrack Input-Parameters", logFile = logFile)

  #Update plotSummary and sizes
  
  plotSummary <- c(plotSummary, external_feature_names, external_loop_names)
  peak_track_size <- 1.5
  loop_track_size <- 3
  single_bulk_track_size <- 1.5
  sizes <-c(sizes, 
                    rep(peak_track_size, length(external_feature_names)), 
                    rep(loop_track_size, length(external_loop_names)))
  
  #Update bulk track size
  if(!is.null(useGroups)) {
      bulkTrackSize <- single_bulk_track_size*length(useGroups)
      sizes[1] <- bulkTrackSize
  }
  ##########################################################
  ...
  ggList <- lapply(seq_along(region), function(x){
    ...
    ##########################################################
    # NEW: External Feature Tracks: mutiple of them! (Peak Set)
    ##########################################################
    for(i in c(1:length(external_feature_names))) {
      palette <- NULL
      external_feature_name <- external_feature_names[i]
      external_feature <- external_features[[i]]
    
      if(!is.null(external_feature)){
            if(str_detect(external_feature_name, "HiC")) {
                    #palette <- paletteDiscrete(set = "stallion", values = c("NeuN+_HiC", "NeuN-_HiC"))
                    palette <- c("NeuN+ HiC"="#7cd67f","NeuN- HiC"="#fba317") #"#D51F26" "#272E6A" 
                    #For some reason these are being coloured the same... fix this (loops are OK)
            }
            else if(str_detect(external_feature_name, "sSNP")){
              palette <- "red4" 
            }
            else if(str_detect(external_feature_name, "LD")){
              palette <- c("Lead SNP"="darkblue","LD SNP"="cyan3")
            }
            else{
              palette <- NULL
            }
             
            .logDiffTime(sprintf("Adding External Feature Tracks (%s of %s)",x,length(region)), t1=tstart, verbose=verbose, logFile=logFile)
            plotList[[external_feature_name]] <- .featureTracks(
                features = external_feature, 
                region = region[x], 
                pal = palette,
                baseSize = baseSize,
                facetbaseSize = facetbaseSize,
                hideX = TRUE, 
                title = external_feature_name,
                highlight = highlight,
                highlightFill = highlightFill,
                logFile = logFile) + theme(plot.margin = unit(c(0.1, 0.75, 0.1, 0.75), "cm"))
      }
    ...
    ##########################################################
    # NEW: External Loop Track (assume HiC)
    ########################################################## 
    for(i in c(1:length(external_loop_names))) {
      palette <- NULL
      external_loop_name <- external_loop_names[i]
      external_loop <- external_loops[i]
      if(str_detect(external_loop_name, "HiC")) {
        #palette <- paletteDiscrete(values=c("neg","pos"), set="stallion")#
        palette <- c("0"="#fba317","1"="#7cd67f")
      }
     
      if(!is.null(external_loops)) { #edit this
          .logDiffTime(sprintf("Adding External Loop Tracks (%s of %s)",x,length(region)), t1=tstart, verbose=verbose, logFile=logFile)
          plotList[[external_loop_name]] <- .loopTracks(
              loops = external_loop, 
              region = region[x], 
              baseSize = baseSize,
              facetbaseSize = facetbaseSize,
    ...
    #plotList <- plotList[order(plotSummary)]
...
.groupRegionSumArrows <- function(
  ...
  plotDF <- plotDF[order(plotDF$group, plotDF$x),]
  plotDF <- plotDF[,c("x", "y", "group")]
...
.geneTracks <- function(
  ...
  geneWidth = 2, 
  exonWidth = 4, 
  labelSize = 5, #CHANGE THIS - originally 2
  facetbaseSize,
...
.featureTracks <- function(
  ...
  baseSize = 9, 
  facetbaseSize = NULL,
  featureWidth = 2,  #see if this makes the peaks thicker on the plot
  borderWidth = 0.4, 
  hideX = FALSE, 
  hideY = FALSE,
  highlight = NULL,
  ...
  if(!is.null(features)){
    ...
    featureList <- featureList[seq_along(featureList)] #removed rev
    ...
    if(is.null(pal)){
      pal <- paletteDiscrete(set = "stallion", values = rev(unique(paste0(featureO$name)))) #changed from set="stallion"
      #pal <- c("tan4") 
      print(pal)
    }
  ...
  if(length(highlight) > 0){
    ...
    p <- p +
      geom_rect(data = dfH, aes(xmin = start, xmax = end, ymin = -Inf, ymax = Inf), 
        alpha=0.2, fill=highlightFill, inherit.aes = FALSE) #change highlightFill... two different colors?
...
.loopTracks <- function(
  ...
  if(!is.null(loops)){
    ...
    valueMin <- min(unlist(lapply(loops, function(x) min(abs(x$value))))) #change color to absolute value of linkage
    #ensures that negative correlations are visualized and have color that corresponds to the magnitude of association
    valueMax <- max(unlist(lapply(loops, function(x) max(abs(x$value)))))
    ...
    if(testDim){
      ...
      loopO$facet <- title
      if(is.null(pal)){
        #pal <- colorRampPalette(c("#3A97FF","#8816A7","black"))(100)
        pal <- colorRampPalette(c("#E0ECF4" ,"#9EBCDA", "#8C96C6", "#8C6BB1", "#810F7C", "#4D004B"))(100)
      }

      p <- ggplot(data = data.frame(loopO), aes(x = x, y = y, group = id, color = abs(value))) + #abs 
        geom_line() +
        facet_grid(name ~ .) +
        ylab("") + 
        ...
        theme(legend.text = element_text(size = baseSize)) + scale_fill_discrete(name = "value") +
